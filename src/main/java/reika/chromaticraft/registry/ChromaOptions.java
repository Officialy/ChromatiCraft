...
/**
 * ChromatiCraft's config options (read via {@code ChromatiCraft.config.getControl(ordinal)} on the
 * {@link reika.chromaticraft.ChromaConfig} built on DragonAPI's ControlledConfig).
 *
 * <p>Port note: the legacy vanilla-dye-chance helpers ({@code doesVanillaDyeDrop}/{@code doesTreeDyeDrop}
 * /{@code isVanillaDyeMoreCommon}) are deferred with {@code ChromaConfig.getVanillaDyeChance} (dye-tree
 * subsystem — nothing in the network/casting path uses them).
 */
public enum ChromaOptions implements SegmentedConfigList, SelectiveConfig, IntegerConfig, BooleanConfig, DecimalConfig, StringConfig, MatchingConfig, CustomCategoryConfig, UserSpecificConfig, Dependency {
	...
	public static int getMaxFenceSections() {
		return ReikaMathLibrary.ceilPseudo2Exp(Mth.clamp(FENCEHOPS.getValue(), 16, 256));
	}

	public static int getStructureTriesPerChunk() {
		return Mth.clamp(STRUCTTRIES.getValue(), 5, 100);
	}
